...
8. **Baseline the test failures and lint/type state.** Using the commands recorded in sub-point 7, run `TEST` once and record the names of any tests that already fail on the unmodified working tree. Then run `LINT`, `FORMAT_CHECK`, and `TYPE_CHECK` once and record any pre-existing failures, with the offending files/rules, not just a pass/fail verdict. These are pre-existing problems unrelated to this implementation. Save both lists for use in Step 5e and Step 10. Any slot recorded as `none — no baseline` in sub-point 7 is skipped.
