...
7. **Detect the project's tooling commands.** Before running anything, identify how this project runs **tests, lint, format-check, type-check**, and (where relevant) **build**. Inspect in this order:
   ...
   Record the resolved commands as named slots — `TEST`, `LINT`, `FORMAT_CHECK`, `TYPE_CHECK`, and `BUILD` (optional). Reuse them by name through sub-point 8, Step 5e, Step 5h, and Step 7 so command choice stays consistent across stages. Reference examples (adapt to what the project actually uses):
   - **Python (uv):** `TEST="uv run pytest -q"`, `LINT="uv run ruff check ."`, `FORMAT_CHECK="uv run ruff format --check ."`, `TYPE_CHECK="uv run mypy"`. Swap `uv run` for `poetry run` / `hatch run` if those are configured instead.
   - **Swift (SwiftPM):** `TEST="swift test"`, `LINT="swiftlint"`, `FORMAT_CHECK="swift-format lint -r Sources Tests"`, `BUILD="swift build"`.
   - **Swift (Xcode):** `TEST="xcodebuild test -scheme <S> -destination <D>"`, `BUILD="xcodebuild -scheme <S> build"`; prefer the project's fastlane lane or `Makefile` wrapper if present.
   ...
   If a slot has no configured tooling for this project, set it to `none — no baseline` and skip the corresponding action everywhere downstream.

8. **Baseline the test failures and lint/type state.** Using the commands recorded in sub-point 7, run `TEST` once and record the names of any tests that already fail on the unmodified working tree. Then run `LINT`, `FORMAT_CHECK`, and `TYPE_CHECK` once and record any pre-existing failures, with the offending files/rules, not just a pass/fail verdict. Save both lists for use in Step 5e and Step 10. Any slot recorded as `none — no baseline` in sub-point 7 is skipped. The recorded **test**-failure list also drives the pre-implementation gate in sub-point 9: a non-empty test baseline halts the skill (the user decides what to do) rather than being silently carried into the stage loop. The lint/type list, by contrast, is the set of pre-existing problems excluded from regression gating (it does not block — see the next paragraph).
...
Before committing, review only the diff introduced by this stage. Run all of these lenses; fix what you find via `Edit` in the same stage:
...
## Step 7 — Final design coverage check
...
If anything is missing, add a final lightweight stage (named e.g. *Stage N+1 — Coverage gaps*) following the same TDD cycle to close the gap, and commit it the same way. Update the plan file to reflect the added stage.
