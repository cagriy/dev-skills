...
**Verify the design's claims, not just its citations.** Confirming that a cited file or symbol *exists* is the weakest available check, and it is the one that keeps passing while the plan inherits a defect. Four classes need you to read the code behind the claim:

- **Names the design says it will add.** For every symbol, file, or key the design says the feature will add, gain, or extend, grep for an existing definition of that exact name. A same-name/different-purpose collision is a Step 7 conflict to raise, not a coincidence to design over.
