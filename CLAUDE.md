...
- `hooks/remote-check.sh` — the only executable code in the plugin. A remote-readiness *warning* (not a gate): it `git fetch`es and, if the current branch is behind its upstream, emits both a user-visible `systemMessage` (a terminal warning, so the behind-state is never silent) **and** an `additionalContext` note so Claude can offer a one-click `git pull --ff-only`. The `systemMessage` is load-bearing: `additionalContext` alone is injected into Claude's context only and is *not* shown to the user, which is why a no-op feature prompt produced no visible feedback before. **SessionStart always fetches and resets the throttle; UserPromptSubmit fetches only when the last check is older than 2h** (state in `<git-dir>/dev-skills-remote-check`, never committed), so it never nags every prompt. No-op outside a git repo, with no upstream, or offline; it never pulls and never blocks. It is a complement to — not a replacement for — checking remote state at the point work happens; a once-or-throttled fetch can't see a remote that advances between checks, so it warns rather than enforces.
