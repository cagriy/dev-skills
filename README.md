...
- A **prompt router** (`feature-dispatch`, model-invoked) that spots non-trivial feature requests in your normal prompts and offers to enter the chain at the right point — `/dev:feature-storm` for vague ideas, `/dev:feature-design` for clear-but-undesigned ones — before any code is written.
...
- An **eval suite**: `evals-code-run` scores your unpushed commits for duplication, bloat, inefficiency, and security (higher = worse); `evals-e2e-run` scores a chain-built feature's artefact quality and stage-to-stage consistency (higher = better). Both append JSON entries to logs under `~/.claude/evals/` and are offered automatically at the end of `/dev:feature-implement`.
- A lightweight **lessons-learned loop**: the feature skills and `bug-fix` append improvement observations to a per-skill log; you periodically apply them with `/dev:lessons-learn`.
...
**[Interactive architecture diagram → `diagram/index.html`](diagram/index.html)** — open it in a browser for the full end-to-end picture (every skill, step, gate, loop-back, and hook). Regenerate it with `/dev:diagram-update` after changing skills.
...
/dev:feature-storm  →  /dev:feature-design  →  /dev:feature-plan  →  /dev:feature-implement
         │                      │                      │                        │
         │                      │                      │                        ├──→ offers /dev:evals-code-run
         ├──────────────────────┴──────────────────────┴────────────────────────┤    + /dev:evals-e2e-run
         ...
         └─────── each one's final step captures one lesson ────────────────────┘
                         ...
                         ~/.claude/dev-skills/lessons/<slug>.md
                                            ↑
                       /dev:lessons-learn <slug> (periodic)
```
...
### `/dev:feature-storm`  *(optional, first step)*
...
- **Chains into:** `/dev:feature-design`.
...
- **Chains into:** `/dev:feature-implement`.

### `/dev:feature-implement`
...
- **Baseline gate:** establishes a test/lint baseline before building; if the test suite is already red it halts and asks whether to investigate, file a bug via `/dev:bug-submit`, or stop — it never builds on a red suite.
...
### `/dev:bug-submit`  *(standalone — not part of the feature chain)*
...
### `/dev:evals-code-run [base-ref]`  *(standalone, read-only)*
...
- **Use when:** you've just landed commits (typically via `/dev:feature-implement` or `/dev:bug-fix`) and want a quality read before pushing. Also offered automatically by `/dev:feature-implement`'s closing step.
...
### `/dev:evals-e2e-run [base-ref]`  *(standalone, read-only)*
...
- **Use when:** `/dev:feature-implement` has just finished and you want to know how well the pipeline held together. This is the expected initiator; it's also offered automatically by `/dev:feature-implement`'s closing step.
...
### `/dev:diagram-update`  *(maintenance — runs only inside this repo)*
...
### `/dev:lessons-learn <skill-name>`  *(maintenance, user-only)*
...
The first command adds the [`ai-tools` marketplace](https://github.com/cagriy/ai-tools); the second installs the `dev-skills` plugin from it. The plugin's manifest name is `dev`, so its slash commands are namespaced `/dev:` (e.g. `/dev:feature-design`).
...
/dev:feature-storm Add reminders to the todo app
...
Each step can also be run standalone with an explicit version, e.g. `/dev:feature-design v3`.
...
templates/workflow-diagram.html  HTML template /dev:diagram-update renders the architecture diagram from
