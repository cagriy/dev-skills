...
- A **standalone bug-submit** skill that files a triaged GitHub issue (with optional image attachments) and adds an initial triage comment grounded in a quick read of the codebase.
...
Files a GitHub issue against the current repo for a bug, then posts a triage comment with the initial understanding of the problem, grounded in a quick read of the relevant code paths.

- **Use when:** you've just hit a bug and want a clean, triaged issue without leaving the terminal.
...
- **Outputs:** one GitHub issue + one triage comment. Images, if any, are uploaded as a single secret gist and embedded.
- **Step 0 confirmation is non-negotiable** because this skill posts public content to GitHub.
